...
$$\theta = \int_0^\infty \int_0^x g(x,y) dy dx = \int_0^\infty  \int_0^\infty g(x,y) \ \mathbb{I}{(y)}_{(0,x)} dy dx$$
...
Vamos agora calcular (deixei esse aqui como exemplo pros dois de baixo era bom substituir g pela funcao no segundo passo)
$$\int_0^\infty \int_0^x \exp^{-(x+y)} dydx = \int_0^1  \int_0^1 g(u,v) \ \mathbb{I}{(v)}_{(0, u)} \frac{1}{(1 - u)(1 - v)} dv du$$
